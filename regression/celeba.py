...
def main():
    ...
    if args.expid is not None:
        args.root = osp.join(results_path, 'bound', 'celeba', args.model, args.expid)
    else:
...
def train(args, model):
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for (x, _) in tqdm(train_loader, ascii=True):
            ...
            inco_bound = np.sqrt((outer_g_inco_bound) / (args.train_batch_size * args.num_epochs * len(train_loader) * 10))
...
def eval_all_metrics(args, model):
    ...
    model.eval()
    with torch.no_grad():
        ravgs = [RunningAverage() for _ in range(3)]
        for batch in tqdm(eval_batches, ascii=True):
            for key, val in batch.items():
                batch[key] = val.cuda()
...
def plot(args, model):
    ...
    save_dir = osp.join(results_path, 'celeba', 'plots', f'{args.plot_seed}', args.model, args.expid)
